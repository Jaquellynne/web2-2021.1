<script>
    $(document).ready(function() {
        ...
        $('#finalizar').click(function() {
            let vendaTotal = $(`#vendaTotal`).val().split(' ');
            if (parseFloat(vendaTotal[1]) === 0) {
                Swal.fire({
                    icon: 'info',
                    title: 'Não existe nenhum produto nesta venda !!',
        ...
        $('#addprod').click(function() {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo e(route('venda.buscaprod')); ?>",
                data: {
    ...
    function trazValor(id) {
        ...
        if (produto >= 2) {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo e(route('venda.buscapreco')); ?>",
                data: {
                    "_token": "<?php echo e(csrf_token()); ?>",
                    produto_id
                },
                success: function(produto) {
                    ...
                    $(`#qtd${id}`).attr('max', produto.quantidade);
                    calcTotal(id);
                }
            });
    ...
    function calcTotal(id) {
        ...
        if (produto_id == '') {
            ...
            let max = parseInt($(`#qtd${id}`).attr('max'));

            if (qtd > max) {
                Swal.fire({
                    icon: 'info',
                    title: 'Só temos ' + max + ' em estoque !!',
                    confirmButtonColor: '#063970',
                });
                $(`#qtd${id}`).val(max)
                qtd = max;
    ...
    function calcTotalGeral() {
        ...
        $(`#vendaTotal`).val("R$ " + valorformat);

    }
</script>
<?php /**PATH C:\laragon\www\projetos\systemcar\resources\views/pages/venda/scripts/formataProdutos.blade.php ENDPATH**/ ?>